...
import { signupValidator, loginValidator, forgotPasswordValidator, resetPasswordValidator, changePasswordValidator } from '#validators/user'
...
import { TenantStatus, UserStatus } from '../Helpers/ENUM.ts'
...
export default class AuthController {
    ...
    async forgotPassword({ request, response }: HttpContext) {
        ...
        const token = jwt.default.sign(
            { user_id: user.uuid, email: user.email },
            secret,
            { expiresIn: '15m' }
        )
    ...
    async resetPassword({ request, response }: HttpContext) {
        const { id, token, password, confirmPassword } = await request.validateUsing(resetPasswordValidator)
